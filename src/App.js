...
function App() {
    ...
    const fetchWeatherData = async (placeData) => {
        const resp = await fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${placeData.results[0].geometry.location.lat}&lon=${placeData.results[0].geometry.location.lng}&appid=3e2bd5f93177f236d0f300f96299a06c`);
        const data = await resp.json();
        console.log(data);
        //setWeatherData([data]); 
    };
